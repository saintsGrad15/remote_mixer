...
def init_midi():
    ...
    try:
        ...
        midi_interface_out = config.get("midi_interface_out", "Remote Mixer IAC Driver")
        port_index = None
        ...
        if port_index is not None:
            ...
            print(f"Opened MIDI output port: {available_ports[port_index]}")
        else:
            print(
            ...
            midi_out.open_virtual_port(midi_interface_out)
            print(f"Opened virtual MIDI output port: {midi_interface_out}")

        # Initialize MIDI Input
        midi_in = rtmidi.MidiIn()
        available_in_ports = midi_in.get_ports()

        print(f"Available MIDI input ports: {available_in_ports}")

        # Find the configured MIDI input interface
        midi_interface_in = config.get("midi_interface_in")
        if midi_interface_in:
            port_index = None

            for i, port_name in enumerate(available_in_ports):
            ...
            if port_index is not None:
                ...
                print(f"Opened MIDI input port: {available_in_ports[port_index]}")
            else:
                print(
                    f"Warning: MIDI input interface '{midi_interface_in}' not found. Available ports: {available_in_ports}")
                # Open virtual port as fallback
                midi_in.open_virtual_port(midi_interface_in)
                print(f"Opened virtual MIDI input port: {midi_interface_in}")
        else:
            print("No MIDI input interface configured in config.json")
    ...
    if config:
        for group in config.get("channel_groups", []):
            for channel in group.get("channels", []):
                for cc_field in ["volume_cc", "mute_cc", "verb_cc"]:
                    cc = channel.get(cc_field)
        ...
        try:
            ...
            if msg:
                ...
                if len(message) >= 3 and (message[0] & 0xF0) == 0xB0:
                    ...
                    if cc in cc_map:
                        print(f"Received MIDI CC: {cc=}, {value=}")

                        # Send update to all connected clients
                        update_message = json.dumps({ "type": "midi_cc_update", "cc": cc, "value": value })
...
midi_listener = threading.Thread(target=midi_listener_thread, daemon=True)
midi_listener.start()
    ...
    while True:
        try:
            ...
            try:
                ...
                elif data.get("type") == "healthcheck":
                    ws.send(json.dumps({
                        "status": "ok",
...
def catch_all(requested_path):
    ...
    if requested_path.startswith("static/"):
        ...
        asset_path = requested_path[len("static/"):] if requested_path.startswith("static/") else requested_path
        return send_from_directory("static", asset_path)

    # Don't intercept API or websocket routes
    if requested_path.startswith("config") or requested_path.startswith("ws"):
